  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button className="absolute left-4 top-4" variant="outline">Plot Options</Button>
      </SheetTrigger>
      <SheetContent side='left' className='w-1/3 sm:max-w-full'>
        <SheetHeader>
          <SheetTitle>Plot Options</SheetTitle>
          <SheetDescription>
            Adjust scale, colors, and other plot options.
          </SheetDescription>
        </SheetHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input 
                ...
                onChange={(event) => setPlotState({...plotState, minScore: Number(event.target.value)})}
                type="number"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
                <Checkbox 
                    ...
                    onCheckedChange={(event) => setPlotState({...plotState, autoMinScore: Boolean(event)})}
                ></Checkbox>
                <label
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input 
                ...
                onChange={(event) => setPlotState({...plotState, maxScore: Number(event.target.value)})}
                type="number"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
                <Checkbox 
                    ...
                    onCheckedChange={(event) => setPlotState({...plotState, autoMaxScore: Boolean(event)})}
                ></Checkbox>
                <label
                    htmlFor="maxColorAuto"
                    className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input 
                ...
                onChange={(event) => setPlotState({...plotState, pointSize: Number(event.target.value)})}
                type="number"
                step={0.1}/>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Checkbox 
                ...
                onCheckedChange={(event) => setPlotState({...plotState, toggleGridlines: Boolean(event)})}
            ></Checkbox>
          </div>
        </div>
        <SheetFooter>
